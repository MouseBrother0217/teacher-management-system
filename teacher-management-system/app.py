...
# 数据库配置
from models import db, Teacher, Course, ClassInfo, TeachingRecord
import os
...
def courses_list():
    ...
    db_courses = query.order_by(Course.id.desc()).all()
    
    # Get teacher associations for each course
    courses_data = []
    for course in db_courses:
        # Query teachers for this course
        teachers_list = []
        try:
            import sqlite3
            conn = sqlite3.connect('instance/teacher_system.db')
            c = conn.cursor()
            c.execute('''
                SELECT teacher_name FROM course_teachers 
                WHERE course_id = ? 
                LIMIT 3
            ''', (course.id,))
            teachers_list = [row[0] for row in c.fetchall()]
            conn.close()
        except:
            pass
        ...
        courses_data.append({
            ...
            'name': course.name,
            'category_name': '其他',
            'teacher_name': '、'.join(teachers_list) if teachers_list else '-',
...
def schedules_list():
    ...
    query = db.session.query(
        ...
        ClassInfo.class_type.label('class_type'),
        Teacher.name.label('teacher_name')
    ).outerjoin(Course, TeachingRecord.course_id == Course.id
    ).outerjoin(ClassInfo, TeachingRecord.class_id == ClassInfo.id
    ).outerjoin(Teacher, TeachingRecord.teacher_id == Teacher.id)
    
    if selected_class:
        query = query.filter(TeachingRecord.class_id == selected_class)
    if selected_teacher:
        query = query.filter(TeachingRecord.teacher_id == selected_teacher)
    if selected_date:
        try:
            date_obj = datetime.strptime(selected_date, '%Y-%m-%d').date()
            query = query.filter(TeachingRecord.teaching_date == date_obj)
        except:
            pass
    
    results = query.order_by(TeachingRecord.teaching_date.desc()).limit(100).all()
    
    # 转换为模板需要的数据格式
    schedules_data = []
    for record, course_name, class_name, class_type, teacher_name in results:
        schedules_data.append({
            'id': record.id,
            'teaching_date': record.teaching_date,
            'start_time': record.start_time,
            'end_time': record.end_time,
            'subject': course_name or '未安排课程',
            'class_name': class_name or '未安排班级',
            'class_type': class_type or '-',
            'teacher_name': teacher_name or '待安排',
            'venue': record.venue or '-',
            'student_count': record.student_count or 0,
            'evaluation_rate': record.evaluation_rate or 0,
            'checkin_rate': record.checkin_rate or 0,
            'score': record.score or 0,
            'lecture_fee': record.lecture_fee or 0,
            'duration_type': record.duration_type or '半天',
            'status': '已完成' if record.score else '待上课'
        })
    
    # 获取班级和教师列表（用于筛选）
    classes_list = [{'id': c.id, 'name': c.name} for c in ClassInfo.query.limit(100).all()]
    teachers_list = [{'id': t.id, 'name': t.name} for t in Teacher.query.limit(100).all()]
